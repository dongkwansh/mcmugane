...
class AlpacaBroker:
    ...
    def __init__(self, settings: Settings):
        ...
        if self.enabled:
            try:
                ...
                logger.info(f"Alpaca 클라이언트 초기화 성공 (계정: {settings.current_account}, 모드: {settings.mode})")
    ...
    def is_account_ok(self) -> bool:
        """계정 정보 조회를 통해 API 키 유효성을 검사합니다."""
        if not self.enabled:
            return False
        try:
            ...
            # 401 또는 403 오류는 명백한 인증 실패입니다.
            if e.code in [401, 403]:
                 logger.error(f"Alpaca 계정 인증 실패 (HTTP {e.code}): API 키가 유효하지 않거나 권한이 없습니다.")
                 return False
            logger.error(f"Alpaca 계정 확인 중 API 오류 발생: {e}")
            return False # 다른 API 오류도 실패로 간주
        except Exception as e:
            logger.error(f"Alpaca 계정 확인 중 예상치 못한 오류 발생: {e}")
            return False
    ...
    def latest_price(self, symbol: str) -> Optional[float]:
        ...
        try:
            ...
            price = trade.get(symbol).price if isinstance(trade, dict) else trade.price
