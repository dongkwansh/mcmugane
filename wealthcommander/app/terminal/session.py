...
class TerminalSessionManager:
    ...
    async def handle_line(self, line: str) -> str:
        ...
        # 명령어 히스토리에 추가
        if s and (not self.command_history or self.command_history[-1] != s):
    ...
    async def _handle_wizard(self, s: str) -> str:
        ...
        if stage == "ask_ticker":
            m = TICK.fullmatch(s)
            if not m:
                return "잘못된 형식입니다. 티커는 .AAPL 형식으로 입력하세요:"
            ...
            if m_num:
                self.wizard["qty"] = float(m_num.group("num"))
            elif m_pct:
                self.wizard["bp_pct"] = float(m_pct.group("pct"))
            elif m_usd:
                self.wizard["budget_usd"] = float(m_usd.group("usd"))
            else:
                return "잘못된 형식입니다. 예: 10 | 20% | $1000"
            ...
            if s:
                try:
                    self.wizard["limit_price"] = float(s)
                except ValueError:
                    return "가격은 숫자여야 합니다:"
            
            # 명령 실행
            d = {
                "cmd": self.wizard["action"],
                "target": "TICKER",
                **self.wizard
            }
            self._reset_wizard()
