...
function appendTerm(s) {
  ...
  } else if (s.startsWith('[시스템]')) {
    pre.style.color = '#8b5cf6';
  }
...
async function loadAccountInfo() {
  try {
    ...
    // 계좌 정보 업데이트
    el('buyingPower').textContent = '$' + (data.buying_power || '-');
    el('equity').textContent = '$' + (data.equity || '-');
    el('portfolioValue').textContent = '$' + (data.portfolio_value || '-');
    ...
    if (clock.timestamp) {
      ...
      el('serverTime').textContent = serverTime.toLocaleTimeString('ko-KR');
    }
...
function sendCmd() {
  ...
  try {
    ...
    // 히스토리에 추가 (중복 제거)
    if (historyBuf.length === 0 || historyBuf[historyBuf.length - 1] !== v) {
      historyBuf.push(v);
