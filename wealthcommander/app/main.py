...
async def post_settings(req: dict):
    ...
    if "language" in req:
        ...
        settings.colors = get_market_colors(req["language"])
        save_settings(settings)
        await broadcast_status_update()
    
    return {"ok": True, "language": settings.language}
...
async def get_accounts():
    ...
    for name, account in settings.accounts.items():
        accounts_list.append({
            ...
            "type": account.type,
            "is_current": name == settings.current_account
        })
    
    return {"accounts": accounts_list, "current": settings.current_account}
