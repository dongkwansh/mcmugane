...
from config import load_settings, save_settings, list_strategies, Settings, get_market_colors
...
def get_current_status_payload():
    ...
    return {
        ...
        "current_account": settings.current_account,  # 추가
        "accounts": list(settings.accounts.keys())    # 추가
    }
...
async def switch_account(request: dict):
    ...
    if account_name not in settings.accounts:
        raise HTTPException(400, f"계좌를 찾을 수 없습니다: {account_name}")
...
async def get_accounts():
    ...
    for name, acc in settings.accounts.items():
        accounts_info.append({
            ...
            "current": name == settings.current_account
        })
