...
def push_system(line: str):
    ...
    # 모든 WebSocket 클라이언트에게 시스템 메시지 전송
    asyncio.create_task(broadcast_system_message(line))

async def broadcast_system_message(msg: str):
    """모든 연결된 WebSocket에 시스템 메시지 브로드캐스트"""
    disconnected = []
    for ws in STATE.websockets:
        try:
            await ws.send_text(f"[시스템] {msg}")
        except:
...
def api_account_info():
    ...
    try:
        ...
        info = {
            ...
            "buying_power": f"{float(acc.get('buying_power', 0)):,.2f}",
            "portfolio_value": f"{float(acc.get('portfolio_value', 0)):,.2f}",
            "equity": f"{float(acc.get('equity', 0)):,.2f}",
...
=============================================
    ...
    async def _cmd_positions(self):
        ...
        for pos in positions:
            ...
            await self.send(
                f"{color} {symbol}: {qty:.4f}주 @ ${avg_price:.2f}\n"
                f"   현재가: ${current_price:.2f} | 평가: ${market_value:.2f}\n"
                f"   손익: {pl_symbol}${abs(unrealized_pl):.2f} ({pl_symbol}{pl_pct:.2f}%)"
            )
        
        await self.send("=================================")
        pl_symbol = '+' if total_pl >= 0 else ''
        await self.send(f"총 평가: ${total_value:.2f} | 총 손익: {pl_symbol}${abs(total_pl):.2f}")
    ...
    async def _cmd_list_myetf(self):
        ...
        for name in myetf_files:
            ...
            if valid and data:
                assets = data.get('assets', [])
                symbols = [a['symbol'].lstrip('.') for a in assets[:3]]
                symbols_str = ', '.join(symbols)
                if len(assets) > 3:
                    symbols_str += f" 외 {len(assets)-3}개"
                
                await self.send(f"✅ {name}: {symbols_str}")
            else:
                await self.send(f"❌ {name}: {error}")
        
        await self.send("================================")
        await self.send(f"총 {len(myetf_files)}개 myETF")
        await self.send("💡 사용법: buy {name} $금액")
    ...
    async def _cmd_orders(self):
        ...
        await self.send("=================================")
        await self.send(f"총 {len(orders)}개 주문 | 'cancel' 명령으로 취소 가능")
    ...
    async def _show_numbered_orders(self, orders: List[Dict[str, Any]]):
        ...
        for i, o in enumerate(orders, 1):
            ...
            await self.send(
                f"  [{i}] {symbol} {side} {qty}주 {price_str}\n"
                f"      상태: {status}{fill_str} | ID: {order_id}\n"
                f"      시간: {created_at}"
            )
    ...
    async def _cmd_history(self):
        ...
        for a in acts[:10]:
            trans_time = a.get('transaction_time', '')[:19]
            symbol = a.get('symbol', '')
            side = a.get('side', '').upper()
            qty = a.get('qty', '')
            price = a.get('price', '')
            await self.send(f"[{trans_time}] {symbol} {side} {qty}주 @ ${price}")
        await self.send("==================================")
    ...
    async def _cmd_ticker(self, sym: str):
        ...
        try:
            ...
            # 출력 포맷
            await self.send(f"========== {sym} ==========")
            await self.send(f"현재가: ${last:.2f} ({'+' if change >= 0 else ''}{change:.2f}, {'+' if change_pct >= 0 else ''}{change_pct:.2f}%)")
            ...
            if pos:
                ...
                await self.send(f"보유: {qty:.4f}주 / 평단: ${avg_price:.2f}")
                await self.send(f"평가: ${market_value:.2f} / 손익: ${unrealized_pl:.2f} ({'+' if pl_pct >= 0 else ''}{pl_pct:.2f}%)")
            else:
                await self.send("보유: 없음")
    ...
    async def _handle_pending_buy_sell(self, flow: str, step: str, user_input: str):
        ...
        if step == 'size':
            ...
            if self.pending.get('is_myetf'):
                ...
                await self.send("목표 가격을 입력하세요 (Enter로 현재가 사용):")
        ...
        if step == 'price':
            ...
            if p:
                try:
                    limit_price = float(p)
                    if limit_price <= 0:
                        await self.send("❌ 0보다 큰 가격을 입력하세요.")
                        await self.send("다시 입력하세요:")
                        return
                    self.pending['limit_price'] = limit_price
                except ValueError:
                    await self.send("❌ 올바른 숫자를 입력하세요.")
                    await self.send("다시 입력하세요:")
                    return
            else:
    ...
    async def _confirm_pending_order(self, flow: str):
        ...
        if target.startswith('.'):
            ...
            await self.send(f"종목: {sym}")
            await self.send(f"구분: {side}")
            await self.send(f"수량: {qty:.4f}주")
            await self.send(f"가격: ${price:.2f} {'(현재가)' if limit_price is None else '(지정가)'}")
            await self.send(f"총액: ${total:.2f}")
            await self.send("===============================")
            ...
            await self.send(f"myETF: {data.get('name', target)}")
            await self.send(f"구분: {'매수' if flow=='buy' else '매도'}")
            await self.send(f"총 투자금액: ${notional:.2f}")
            ...
            for a in assets:
                ...
                await self.send(f"  - {sym}: {weight}% (약 ${alloc:.2f})")
